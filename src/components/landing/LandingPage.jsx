...
import { ArrowDown, ArrowRight, ArrowUpRight, AudioLines, BarChart3, Check, ChevronDown, Headphones, Languages, MessageCircle, Mic, Phone, PhoneIncoming, PhoneOutgoing, Play, ShieldCheck, Sparkles, Wallet } from 'lucide-react';
import { useLanguage } from '../../context/LanguageContext';
import { demoScenarios, landingContent } from '../../content/landingContent';
import { clients, poytaxtClient, qwattClient } from '../../content/clients';
...
function Waves({ className = '' }) {
    return <div className={`lp-waves ${className}`} aria-hidden="true">{Array.from({ length: 49 }, (_, i) => <i key={i} style={{ '--bar-height': `${12 + Math.abs(Math.sin(i * .71) * Math.cos(i * .23)) * 70}%`, '--delay': `${i * -.083}s` }} />)}</div>;
}
...
function SampleDemo({ scenario, copy, phone, setPhone, onCapture, onComplete, retry }) {
    ...
    return <div className="lp-sample">
        <span className="lp-status"><span />{copy.sampleBadge} · O‘zbekcha</span>
        <div className="lp-sample-art"><div className="lp-orb"><AudioLines size={40} strokeWidth={1.3} /></div><Waves /></div>
        <h3>{scenario.direction === 'incoming' ? copy.recordingLabel : copy.outgoingRecordingLabel}</h3>
        <p className="lp-sample-note">{copy.sampleNote}</p>
        ...
        <button type="button" className="lp-sample-retry" onClick={retry}>{copy.retryLive}</button>
    </div>;
}
...
function DemoSection({ copy, phone, setPhone, onComplete, onSelect, scenarioIndex, onCapture }) {
    ...
    return <section id="live-demo" className="lp-section lp-demo" aria-labelledby="lp-demo-title">
        ...
        <div className="lp-scenario-tabs" role="tablist" aria-label={copy.demoEyebrow}>
            {demoScenarios.map((item, index) => {
                ...
                return <button key={item.id} role="tab" id={`scenario-${item.id}`} aria-selected={scenarioIndex === index} aria-controls="scenario-panel" tabIndex={scenarioIndex === index ? 0 : -1}
        ...
        <div id="scenario-panel" className="lp-demo-panel" role="tabpanel" aria-labelledby={`scenario-${scenario.id}`}>
            <div className={`lp-demo-story lp-demo-story-${scenario.id}`} key={scenario.id}>
                ...
                <div className="lp-conversation-art" aria-hidden="true"><span className="lp-conversation-question">{scenarioCopy.prompt}</span><span className="lp-conversation-answer"><span className="lp-mini-orb" />Syncall AI<Waves /></span></div>
                <span className="lp-language-note"><Languages size={17} />{copy.languages}</span>
            </div>
            <div className="lp-demo-widget" id="demo">
                <VoiceCallWidget key={`${scenario.id}-${attempt}`} scenario={scenario} initialPhone={phone} onPhoneChange={setPhone} onLeadCaptured={onCapture} onComplete={onComplete}
...
function CustomerStories({ copy }) {
    return <section className="lp-section lp-stories" id="customers" aria-labelledby="lp-stories-title">
        ...
        <div className="lp-story-grid">
            <Reveal className="lp-story lp-parking-story"><div className="lp-story-top"><ClientLogo client={poytaxtClient} compact /><PhoneIncoming size={25} /></div><div className="lp-story-result"><strong>{copy.parkingMetric}</strong><p>{copy.parkingLabel}</p></div><p className="lp-story-body">{copy.parkingBody}</p><div className="lp-tags">{copy.parkingTags.map((tag) => <span key={tag}>{tag}</span>)}</div></Reveal>
            <Reveal className="lp-story lp-qwatt-story"><div className="lp-story-top"><ClientLogo client={qwattClient} compact /><PhoneOutgoing size={25} /></div><div className="lp-story-result"><strong>{copy.qwattMetric}</strong><p>{copy.qwattLabel}</p></div><p className="lp-story-body">{copy.qwattBody}</p><div className="lp-tags">{copy.qwattTags.map((tag) => <span key={tag}>{tag}</span>)}</div></Reveal>
        </div><p className="lp-result-note">{copy.resultNote}</p>
    </section>;
}

function ProcessPreview({ tab, selected, copy }) {
    return <div className="lp-preview" key={selected}>
        <div className="lp-preview-toolbar"><span className="lp-window-dots" aria-hidden="true"><i /><i /><i /></span><span>{copy.previewLabel}</span><span className="lp-preview-status"><span />{tab.label}</span></div>
        <div className="lp-preview-body">
            <div className="lp-flow-canvas">
                <div className="lp-flow-start"><Phone size={15} />{tab.nodes[0]}</div><span className="lp-flow-line" />
                <div className="lp-flow-node"><div><Sparkles size={17} /><strong>{tab.nodes[1]}</strong><span className="lp-node-dot" /></div><p>{tab.items[0]}</p><span className="lp-node-placeholder" /><span className="lp-node-placeholder short" /></div>
                <div className="lp-flow-branches"><span /><span /></div>
                <div className="lp-flow-pair"><div className="lp-flow-node"><div><MessageCircle size={16} /><strong>{tab.nodes[2]}</strong></div><Waves /></div><div className="lp-flow-node"><div><Check size={16} /><strong>{tab.nodes[3]}</strong></div><span className="lp-node-placeholder" /><span className="lp-node-placeholder short" /></div></div>
            ...
            <div className="lp-preview-inspector"><div className="lp-inspector-label"><span className="lp-mini-orb" />{tab.panel}</div>
                {selected === 4 ? <div className="lp-chart" aria-hidden="true">{[36, 57, 43, 69, 51, 83, 72, 94].map((height, index) => <i key={index} style={{ '--height': `${height}%`, '--delay': `${index * .08}s` }} />)}</div> : <div className="lp-inspector-orb"><div className="lp-orb">{selected === 1 ? <Mic size={38} /> : selected === 3 ? <ShieldCheck size={38} /> : <AudioLines size={38} />}</div><Waves /></div>}
                <div className="lp-inspector-items">{tab.items.map((item) => <span key={item}><Check size={15} />{item}</span>)}</div>
            </div>
        </div>
    </div>;
}
...
function PlatformShowcase({ copy }) {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(([entry]) => setPlaying(entry.isIntersecting), { threshold: .35 });
    ...
    }, [playing, copy.tabs.length]);
    const tab = copy.tabs[selected];
    const selectTab = (index) => { setPlaying(false); setSelected(index); };
    return <section id="how" className="lp-platform" aria-labelledby="lp-platform-title">
        <Reveal className="lp-centered-heading"><span className="lp-eyebrow">{copy.platformEyebrow}</span><h2 id="lp-platform-title">{copy.platformTitle}</h2><p>{copy.platformIntro}</p></Reveal>
        <div className="lp-showcase" ref={ref} onMouseEnter={() => setPlaying(false)} onFocusCapture={() => setPlaying(false)}>
            <div className="lp-showcase-inner" id="platform-panel" role="tabpanel" aria-labelledby={`platform-tab-${selected}`} tabIndex="0">
                <ProcessPreview tab={tab} selected={selected} copy={copy} />
                <div className="lp-showcase-copy" key={selected}><h3>{tab.title}</h3><p>{tab.body}</p></div>
            </div>
            <div className="lp-platform-tabs" role="tablist" aria-label={copy.platformEyebrow}>{copy.tabs.map((item, index) => <button key={item.label} id={`platform-tab-${index}`} role="tab" aria-controls="platform-panel" aria-selected={selected === index} tabIndex={selected === index ? 0 : -1} onClick={() => selectTab(index)} onKeyDown={(e) => {
                let next;
            ...
            }}><span className="lp-tab-indicator" data-playing={playing && selected === index || undefined} />{item.label}</button>)}</div>
        </div>
    </section>;
}
...
function BusinessForm({ copy, phone, setPhone, language, initialUseCase, product, intent }) {
    ...
    return <section className="lp-contact-wrap" id="contact" tabIndex="-1" aria-labelledby="lp-contact-title"><div className="lp-contact">
        <div className="lp-contact-intro"><span className="lp-eyebrow">SYNCALL AI</span><h2 id="lp-contact-title">{copy.formTitle}</h2><p>{copy.formIntro}</p><a href="https://t.me/syncall_ai" target="_blank" rel="noopener noreferrer">{copy.contactLink}<ArrowUpRight size={17} /></a><div className="lp-contact-art" aria-hidden="true"><Waves /><span>syncall<span className="lp-logo-dot">✳</span></span></div></div>
...
export default function LandingPage() {
    ...
    return <main id="main-content" tabIndex="-1" className="lp">
        <section className="lp-hero" aria-labelledby="lp-hero-title"><div className="lp-hero-glow" aria-hidden="true" /><div className="lp-hero-content"><span className="lp-hero-eyebrow"><span />{copy.eyebrow}</span><h1 id="lp-hero-title">{copy.title[0]}<br /><span>{copy.title[1]}</span></h1><p className="lp-hero-intro">{copy.intro}</p><div className="lp-hero-actions"><a className="lp-button lp-button-dark" href="#live-demo"><AudioLines size={19} />{copy.tryAgent}<ArrowUpRight size={17} /></a><a className="lp-button lp-button-outline" href="#contact">{copy.talkTeam}</a></div><p className="lp-hero-note">{copy.heroNote}</p></div><div className="lp-hero-bottom" aria-hidden="true"><span>01 — SYNCALL VOICE AI</span><Waves /><span>UZBEKISTAN · UZ / RU</span></div><a href="#live-demo" className="lp-hero-scroll" aria-label={copy.tryAgent}><ArrowDown size={20} /></a></section>
        <div className="lp-logos"><p>{copy.trusted}</p><ul>{clients.map((client) => <li key={client.id}><ClientLogo client={client} /></li>)}</ul></div>
        ...
        <section id="capabilities" className="lp-section lp-products" aria-labelledby="lp-products-title"><Reveal className="lp-section-heading"><div><span className="lp-eyebrow">{copy.productsEyebrow}</span><h2 id="lp-products-title">{copy.productsTitle}</h2></div><p>{copy.productsIntro}</p></Reveal><div className="lp-products-grid">{copy.products.map((product, index) => { const Icon = productIcons[index]; return <Reveal key={product.path} className={`lp-product lp-product-${index}`}><span className="lp-product-badge">{product.badge}</span><div className="lp-product-visual" aria-hidden="true">{index === 0 ? <div className="lp-small-chart">{[28, 45, 36, 62, 48, 75, 66, 88].map((height, i) => <i key={i} style={{ height: `${height}%` }} />)}</div> : index === 1 ? <div className="lp-chat-art"><span /><span /><span /></div> : <><Icon size={36} strokeWidth={1.25} /><Waves /></>}</div><Icon size={24} strokeWidth={1.5} /><h3>{product.title}</h3><p>{product.body}</p><Link to={localePath(product.path)}>{product.label}<ArrowUpRight size={17} /></Link></Reveal>; })}</div></section>
